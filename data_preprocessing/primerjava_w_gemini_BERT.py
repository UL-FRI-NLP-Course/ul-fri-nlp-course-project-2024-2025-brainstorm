...
def retrieve_surrounding(selected_timestamp, df_input, traffic_dogodki):
    ...
    if events:
        for event in events:
            ...
            besedilo_za_primerjavo = ""
            for ev in event_data:
                ev_arr = ev.split(':')
                if len(ev_arr)>1 and ev_arr[0] in ['LOKACIJA', 'SMER']:
                    besedilo_za_primerjavo += ev_arr[1]
        
            print(Fore.GREEN + besedilo_za_primerjavo)
            
            i_in_offset = [input_row_index + offset for offset in range(-30, 1)]
...
def calc_similarity(df_input, df_traffic_events):
    for i, row in df_traffic_events.iterrows():
        ...
        traffic_dogodki = row['Prometni_dogodki']
        
        if not 'Error' in traffic_dogodki:
            retrieve_surrounding(timestamp, df_input, traffic_dogodki)

    
calc_similarity(df_input_excel, df_traffic_events)
