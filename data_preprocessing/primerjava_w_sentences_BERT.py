...
start_datum = pd.Timestamp(2024, 2, 1)
...
def get_most_similar_content(df_input, rows_indices, query):   
    ...
    #vectorizer = TfidfVectorizer()
    #tfidf_matrix = vectorizer.fit_transform([text_A, text_content])
    ...
    """for i, idx in enumerate(top_indices):
        print(Fore.BLUE + f"Match #{i+1} (Score: {similarities[idx]:.4f}):\n{documents[idx]}\n")"""
        
    best_match = documents[top_indices[0]]
    top_sim = similarities[top_indices[0]]
    
    print(Fore.BLUE + f"Match: (Score: {top_sim:.4f}):\n{best_match}\n")
    ...
    if events:
        for event in events:
            ...
            if len(besedilo_za_primerjavo) > 0:
                ...
                print()
    best_match_content_all += '"'
    return best_match_content_all
                
                        
def calc_similarity(df_input, df_traffic_events, window_size):
    with open('matched_events3.csv', 'w', newline='', encoding='utf-8') as csvfile:
        ...
        for i, row in df_traffic_events.iterrows():
            ...
            if len(traffic_dogodki_porocilo)>1:
                best_input_za_porocilo = retrieve_surrounding(timestamp, df_input, traffic_dogodki_porocilo[1], window_size)
...
calc_similarity(df_input_excel, df_output, 30)
